...
배포 강의나 Vercel 설정을 다룰 때는 `DEPLOYMENT_LECTURE_SCRIPT.md`와 `DEPLOYMENT_STUDENT_GUIDE.md`를 먼저 확인하세요.
...
- Current files:
  ...
  - `scripts/build-deploy.mjs`: Vercel build step that creates `dist/supabase-config.js` from deployment environment variables
