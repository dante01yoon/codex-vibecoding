...
- `src/components/GuestbookLanding.jsx`를 추가해 히어로, 기능 소개, 사용 흐름, CTA를 한국어 문구로 구성했다.
- 랜딩 CTA는 기존 작성기와 피드 영역으로 `scrollIntoView` 이동하며, 기존 방명록 작성/피드/댓글 상태 흐름은 유지했다.
- 스타일은 기존 CSS 변수와 포스트잇 색감 위주로 맞추고, 모바일에서는 CTA와 흐름 카드가 1열로 접히게 했다.
