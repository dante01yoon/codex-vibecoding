...
## 2026-05-16 랜딩 페이지 추가

`src/components/LandingPage.jsx`를 추가해 히어로, 기능 소개, 사용 장면, CTA를 별도 컴포넌트로 분리했다. `src/App.jsx`는 상단 랜딩 뒤에 기존 방명록 작성/피드 영역을 유지하고, CTA 클릭 시 `#guestbook`으로 스크롤한다. 스타일은 기존 따뜻한 종이 배경, 코랄 CTA, 포스트잇 미리보기 톤을 유지하며 과한 glassmorphism이나 gradient orb는 넣지 않았다.
